...
| 2026-07-26 | `/free-appraisal/` — the last raw YouTube `<iframe>` on the site converted to `lite-youtube` | **This one page sat at PSI mobile 57 while the rest of the site scored in the 90s, and every single finding in its report was YouTube:** 550 KiB unused JS (`base.js` 464 KiB + 2 more), 106 KiB unused CSS (`www-player.css`), and **all 4 long main-thread tasks (427 ms total)**. Cause: it embedded the video as a raw `<iframe src="youtube-nocookie.com/embed/…">`, which boots the entire YouTube player on load. **`loading="lazy"` did NOT save it** — the embed sits high on the page, so it is always inside the load window; lazy defers off-screen work, it does not defer *this*. Every other video page (19 files) already used the `lite-youtube` facade from PR #74; this was the sole holdout. Now `<lite-youtube videoid="uSu-Ld-xgnI" params="wmode=transparent&autoplay=1&enablejsapi=1&rel=0&modestbranding=1">` with the vendored `/styles/lite-yt-embed.css` + `/scripts/lite-yt-embed.js` head-slot pair, identical to `index.astro` and the sell-my-* pages. Verified on a real preview: element upgrades, play button renders, poster loads from `i.ytimg.com`, **0 iframes on load**, and clicking swaps in the real iframe with the right params and `lyt-activated`. Sizing verified at 1920 and 375 (327x184, aspect 1.78, fills its wrapper, no overflow). ⚠️ **The CSS selector had to become `.fa-intro__video-wrap iframe, .fa-intro__video-wrap lite-youtube`** — keep BOTH: lite-youtube injects a plain `<iframe>` as its child on activation, so dropping the iframe half breaks the video's sizing the moment someone presses play. | **Rule: grep for raw `<iframe … youtube` before trusting that a site is "on the facade" — one holdout page can cost ~918 KiB and 40 PSI points on its own, and the sitewide average hides it.** Field CWV for this URL was already passing (LCP 1.2s / INP 152ms / CLS 0); the 57 was a lab score, but the ~918 KiB was real weight on real phones. |
| 2026-07-26 | Sitewide — images bypassing the Astro pipeline, enumerated and the 2 worst fixed | **Swept all 111 sitemap URLs for own-domain raster `<img>` NOT served from `/_astro/`. Result: 63 instances across 61 of 111 pages, but only 7 DISTINCT files, and ZERO of the 63 had a `srcset`.** Two dominated and are now fixed: (1) **the blog author portrait**, `post/[slug].astro` rendered `about-me-joe-1.jpg` via `assetSrc()` — a 587x587 / **81,646-byte** original poured into a **120x120** slot, on **all 57 blog posts**. Now `<Image>` off `resolveImage("images/fender-sn/about-me-joe-1.jpg")`: **3,092 bytes** + a 240w retina variant, **-78.7 KB per post**. (2) **`/rickenbacker-serial-numbers/`** shipped the 4005 Azureglo as a raw `<img width="2560" height="1707">` = **746,562 bytes** into a slot measured live at **860px desktop / 343px mobile**. Now `<LicensedImage width={860}>`: **54,714 bytes, -675.7 KB**. Its `buildImageGraph` entry was REMOVED in the same edit because `LicensedImage` self-licenses — licence gate held at exactly **209 nodes / 0 mismatches**, proving nothing was lost or double-counted. ⚠️ The pipeline twin is at `src/assets/images/1968-rickenbacker-4005-azureglo-scaled.jpg` — unprefixed folder AND missing the `-1` the `public/` copy carries — so the page's own `imgMeta()` helper cannot reach it; it is resolved directly. Both call sites keep a raw-`<img>` fallback if `resolveImage` ever returns null. **STILL BYPASSING (deliberately left):** the 3 Gibson-serial 360x240 thumbs (30/23/20 KB, no `src/assets` twin) and `1956-gibson-les-paul-standard-goldtop-1024x683.jpg` / `1959-martin-d-28-e-1-1024x683.jpg` on the sell pages (153/119 KB, twins live under `src/assets/blog/…` so wiring them means crossing a content-collection boundary). **Do NOT "fix" the one surviving `/images/about-me-joe-1.jpg` string in the built HTML** — it is the global `Person` schema's `ImageObject.url` in `Layout.astro` and MUST stay a stable unhashed public URL; 0 rendered `<img>` use it. | Found while working the PSI report for the Fender hub. **Method worth repeating: sweeping for `<img>` whose `src` lacks `/_astro/` finds this whole class in one pass, and it collapses to a handful of distinct files — a per-page audit would have reported "1 image" 61 times and buried the 57-page one.** |
| 2026-07-26 | Fender hub — the 2 Mexico prefix photos moved from a raw `<img>` back onto `LicensedImage` | The 2026-07-23 hub-and-spoke rebuild (PR #69) wired `mn-prefix.jpg` / `mz-prefix.jpg` as a raw `<img src={img(...)}>`, which ships the **untouched 1200x1600 originals (257 KB + 294 KB = 538 KB, no srcset, no intrinsic width/height)** while every sibling in the same `.figrow` served an optimised WebP. They were also the page's only CLS risk. Now `<LicensedImage width={540}>` like their siblings: a real visitor downloads **104 KB on desktop (-81%) or 155 KB on mobile at DPR2 (-71%)**, with `width`/`height` present. **They ARE Joe's photos and are licensed normally.** ⚠️ Do not be misled by the stale branch `fix/unlicense-third-party-mexico-images` (`ae9a0a2`, 2026-07-15) or by any note saying these are third-party: that was true of the ORIGINAL pair (~301px web-scrapes), and Joe shot and committed replacements the very same day in **`3ebaaad` "Replace MN/MZ Mexico prefix photos on Fender serial guide"**. That branch is obsolete — it un-licenses files that no longer exist — and should be deleted rather than merged. Verified after the swap: `verify-image-license.mjs` 209 nodes / 0 mismatches. | Found during the 2026-07-26 page-by-page speed pass. **Lesson: a provenance note has a shelf life. Before acting on "these images aren't the client's," check `git log --follow` on the actual files — the fix may already have landed and inverted the correct action.** |
| 2026-07-26 | `BlogArchive.astro` — only the FIRST post card loads eagerly (was the first three) | `eager={current === 1 && i < 3}` matched the **desktop** 3-up first row, but the grid stacks **1-up on mobile**, so on a 375x812 screen cards 2 and 3 sit at **1217px and 1717px** — far below the 812px fold — and still loaded eagerly. Measured live: that pulled ~150 KB of off-screen imagery into contention with the LCP element on exactly the connection least able to absorb it. Now `i < 1`. Cards 2 and 3 remain in the desktop first row where an in-viewport `loading="lazy"` image is still fetched during initial layout, so desktop pays nothing. Applies to `/blog/`, its 6 pagination pages, and every `/category/` archive. | Found during the 2026-07-26 page-by-page speed pass. **Rule: an eager/lazy cutoff tuned to a desktop grid is wrong the moment the grid reflows — pick the cutoff from the NARROWEST layout's fold, not the widest.** |
| 2026-07-26 | `/free-appraisal/` — hero preload pointed at the JPEG the page never paints | **A `rel=preload` whose href does not match what CSS actually renders downloads a second copy at the HIGHEST priority.** The desktop preload was `/images/hero-background.jpg` (275 KB) while `PageHero`'s `--hero-bg-set` `image-set()` paints `/images/hero-background.webp` (170 KB) in every browser that supports WebP. Measured live at 1920px: the JPEG was fetched over the network (`initiatorType: "link"`, 275 KB transferred) and **never rendered**, while the WebP it actually paints came from cache. That is 275 KB of dead weight at `fetchpriority="high"`, contending with the real LCP element, on the appraisal money page. Fixed by pointing the preload at the `.webp` + adding `type="image/webp"`, matching what `index.astro:77` already does. **Rule: whenever you preload a CSS-background hero, the `href` must be the exact asset `image-set()` resolves to — a preload does not participate in `image-set()` negotiation.** The other 8 `PageHero` pages were never affected: they ship no preload at all, so `image-set()` alone correctly picks the WebP. | Found during the 2026-07-26 page-by-page speed pass. The homepage preload was corrected in an earlier hero-WebP pass and `/free-appraisal/` was missed, so this was the last page still pulling the 281 KB JPEG. |
| 2026-07-27 | AMP URL retirement + the one migrated slug that broke (`_redirects`) | From the GSC Coverage drilldown (428 URLs, 2026-07-27). **Most of that list is stale, not live breakage** — spot-checks confirmed `/vintage-guitar-resource-center/`, `/sell-my-vintage-guitar`, `/gretsch-serial-numbers/`, `/sell-my-collection/` and `/post/1968-gibson-es335-guide/` all already 301 correctly; GSC reports historical crawls. Real breakdown: **247 query-string** (left to 404 by design), **155 WooCommerce** (410 by design), **6 AMP**, ~10 other. Fixed two real things. (1) **AMP has TWO URL shapes** and only the suffix form was ever handled: `/<path>/amp/` AND `/amp/<path>/` (prefix, confirmed live on `/amp/` the homepage and `/amp/free-appraisal/`). Added explicit rules for the three guides that moved OUT of `/post/` at cutover, then a generic `/post/:slug/amp`, then `/amp` + `/amp/` for the homepage, then an `/amp/*` splat. (2) **`identify-gibson-lg-series`**: the migration prefixed that one post's slug with the domain name (`joesvintageguitarsaz-com-identify-gibson-lg-series`), leaving the original WP URL a 404 with no rule. Verified against the WP DB: of 41 published WP posts this is the ONLY one without a matching Astro slug, and git shows **zero** posts ever deleted or renamed, so nothing was lost. | **ORDER MATTERS in `_redirects`, first match wins.** The explicit guide rules must stay ahead of the generic `/post/:slug/amp`, or those three take a second hop through `/post/<slug>/`; `dating-a-les-paul` must stay ahead of everything because it has no live base page; `/amp/` must precede `/amp/*` so it cannot resolve to an empty splat. A blind `/post/*/amp/ -> /post/:splat/` wildcard would have sent `/post/how-to-read-gibson-serial-numbers/amp/` to a URL that does not exist, turning a 404 into a redirect-to-404. Validated before commit: 66 rules, 0 malformed, 0 duplicate sources, 25 dynamic (CF limit 100). **`_redirects` cannot be tested locally (no wrangler) — verify on the deployed edge after merge.** Also: bulk-curling ~50 URLs in a loop trips Cloudflare rate limiting and returns `000` for every request; test in small batches with a known-good control. |
| 2026-07-27 | `/reverb-reviews/` — `content-visibility: auto` on the 2,052 review cards | **The page already passes Core Web Vitals for real users** (28-day CrUX: LCP 1.3s, INP 152ms, **CLS 0**). The 0.223 CLS / 82 perf in Lighthouse is a cold-load artifact on a throttled Moto G. The real cost is structural: 2,052 `<article class="rv">` inside an **18,175-element DOM**, which is what makes the 361ms forced reflow expensive (third-party scripts touching a huge render tree — **none of our own inline JS reads geometry**, verified: zero `offsetWidth`/`getBoundingClientRect` anywhere in the document). Added `content-visibility: auto; contain-intrinsic-size: auto 140px` to `article.rv` so off-screen cards skip layout and paint. **The fallback was measured live across all 2,052 cards, not guessed: mean 146px at 390px wide, 120px at 1920px.** The `auto` keyword makes the browser remember each card's real height after first render, so the fallback only governs the very first paint. | **Do NOT trim the reviews to chase the lab score.** This page exists so search and AI engines can read all 2,191 reviews (Reverb's own data is not crawlable), and real users already see zero shift. `content-visibility: auto` defers *rendering*, it does not remove content: verified post-build that the raw HTML still ships all 2,052 articles, the DOM holds **449,077 chars** of review text, the deepest off-screen card is readable via `textContent`, and nothing is `hidden` or `aria-hidden`. Gotcha: `innerText` DOES skip this content by design (it reflects rendering), so audit crawlability here with `textContent` or the raw HTML, or you will wrongly conclude the reviews disappeared. |
...
| 2026-07-27 | Video embeds + VideoObject/Clip schema across the video pages (Fender hub + pot-codes + neck-dates, Martin serial guide, free-appraisal, homepage `Guide.astro` shipping video, and the sell-* pages) | **Extended the existing chapter-deep-link pattern site-wide, fixed two wrong timestamps, and added VideoObject + Key Moments schema.** (1) **Fixed timestamps:** pot-codes embed `start=254` → **184** (the "Checking Pots" chapter) and neck-dates `114` → **125** ("Neck Date"), each in all three spots (lite-youtube `params` + VideoObject `embedUrl` + `contentUrl`). (2) **Section deep-link embed** where a section maps to a chapter: Martin — "Where Is The Serial Number Located?" → `start=20`, following the pot-codes pattern (`params="rel=0&modestbranding=1&start=NNN"` + a local poster background via `assetSrc`). The **Fender hub got NO new section embeds** — its Pot-Codes/Neck-Heel cards link to the spoke pages, which already carry the deep-linked embeds (Joe's call); the hub's 0:00 embed was left alone. (3) **VideoObject** (copied the hub shape: `author` → `/about-me/#person`, real `uploadDate` pulled from each video's YouTube page, `maxresdefault` thumbnail) added to the 10 embeds that had none. (4) **Key Moments (`hasPart` Clip[])** added to the Fender (8) and Martin (2) VideoObjects, one Clip per chapter, `url` = page `?t=NNN`. | P1 of the schema/share/video roadmap (`docs/schema-and-share-card-roadmap.md`) wanted VideoObject wherever a video embeds; deep links + Key Moments are an *explicit* signal (Google may already derive Key Moments for YouTube-hosted video, so no rich-result promised). **The Gibson serial guide (`how-to-read-gibson-serial-numbers`) was intentionally EXCLUDED from this PR at Joe's request 2026-07-27** — its section deep-links (a Joe-confirmed 0:12 interior label / 0:28 headstock mapping), VideoObject, and Clips were built then pulled back out, so treat a Gibson video-schema pass as separate future work, NOT an oversight to "fix". **The "Meet Joe" clip (`zMd6IJ-6e-8`, whose real YouTube title is a 1962 ES-335 performance) is named per each page's on-page embed title** ("Meet Vintage Guitar Expert, Joe Dampt"; Martin sell page uses "Introducing Joe Dampt, A Martin Guitar Connoisseur") to match how the site presents it, not the raw title — Joe confirmed keeping these on-page titles 2026-07-27. Gates: `astro check` 0 errors; build 116 pages (117 locally only because the untracked `how-to-spot-a-fake-vintage-fender` draft builds too); `verify-image-license.mjs` 209 nodes / 0 mismatches. |
| 2026-07-31 | `Layout.astro` — CallRail dynamic number insertion restored | **CallRail (company `480262361`) reinstated as its own early-loading, host-gated inline script**, directly above the idle-deferred WhatConverts block. It was never hard-coded here: `GTM-TKN5P7S` loaded it (container `tag_id` 9). When Google Tag Gateway was switched off at the Cloudflare **account** level on 2026-07-26, the container died and took CallRail with it, so **every website-originated call from 07-26 until this shipped was unattributed**. Calls still reached Joe throughout (the un-swapped number is his real line), so nothing was dropped — only the attribution. Snippet was **recovered from the still-published container** (`https://www.googletagmanager.com/gtm.js?id=GTM-TKN5P7S`), not guessed, and the account was confirmed live first (`swap.js` → 200, current build, i.e. billing while doing nothing). | **⚠️ Do NOT move this into the idle-deferred `TAGS` array below it, and do NOT make it a Zaraz tool.** The number swap must complete before a fast visitor dials, or they call the un-swapped number and the lead lands untracked — the 2026-07-01 remediation plan called this out verbatim ("Do NOT defer CallRail"). `async` is correct; `defer`/idle is not. Zaraz cannot do it at all: it runs edge-side and cannot rewrite the DOM. **⚠️ CallRail is NOT redundant with WhatConverts** — WhatConverts' tracking numbers live inside the Google Business Profiles and the Bing call extension, so it only ever sees calls from those surfaces; CallRail is the only thing that has ever tracked calls starting on the website. Removing it does not fall back, it goes dark. **General lesson: a page-source grep cannot see a tag that another tag loads** — grepping for `callrail` returned 0 and read like proof of absence. Before killing a container, inventory what it loads. |
| 2026-08-18 | `/guitar-identifier/` — rebranded **Vintage Guitar AI** and made installable (PWA), scoped to this page only | The tool now carries a product name and an origin story (created by Joe Dampt, answering from his own corrected serial references, illustrated with his own photographs), plus a "How This One Is Different" section below the chat. The photo-privacy line was removed at Josh's request; the free-appraisal CTA in that same paragraph was kept. **Installable via `public/guitar-identifier/manifest.webmanifest` + `sw.js`, both deliberately living UNDER `/guitar-identifier/` so the service worker's default scope is that one path.** Verified: the SW controls `/guitar-identifier/` and does NOT control `/free-appraisal/`, and no other page links the manifest. Icons generated from the 1440x1440 `joes-vintage-guitars-logo.jpg` (192/512/maskable-512/apple-touch 180); the maskable one insets the logo to 60% for Android's safe-zone crop. `_headers` gains `Cache-Control: no-cache` on `sw.js` and an explicit `application/manifest+json` content type. | Josh's call 2026-08-18. **⚠️ The SW must never cache `/api/identify`** — it is a POST to Claude, every answer is unique, and a stale reply is worse than none; the fetch handler returns early for non-GET *and* for any `/api/` path, verified empirically (POST + GET both left zero cache entries). **⚠️ It also does not precache hashed `/_astro/` bundles by name**, since those filenames change every build and a stale precache list would pin an installed app to an old build; they are runtime-cached instead. Bump `CACHE_VERSION` in `sw.js` to force clients onto a fresh cache. |
| 2026-08-18 | `/guitar-identifier/` — mobile input, tap targets, and a Send button the floating widget was covering | At 375px the textarea was squeezed to **181x42** between the attach and Send buttons, and all three controls sat under the 44px tap-target minimum. The input now takes its own full-width row (**319x56 at 17px**) with the buttons beneath, and auto-grows to a 30vh cap. Also fixed: the site's floating contact widget (`aside.fixed.bottom-6.right-4`, z-index 40) overlapped the right half of Send, so part of it was untappable — Send now reserves a `2.75rem` right gutter to clear it (and the widget is hidden outright in installed/standalone mode). **That overlap pre-dates this work and is present on the current `staging` build.** Separately, the page was the only one on the site rendering a **nested `<main>`** inside Layout's `<main id="main-content">`; it is now a `div`. | **⚠️ GOTCHA worth remembering: an empty `<textarea>` reports its PLACEHOLDER's wrapped height in `scrollHeight`, not one line.** Measured at 375px: empty reported **72px** while one line of real text reported **48px**, because the old 45-character placeholder wrapped to two lines. A textbook `height=auto; height=scrollHeight` auto-grow therefore leaves the field permanently oversized after every send. `autoGrow()` sets no inline height at all when the value is empty and lets the stylesheet's `min-height` own the resting size. The placeholder was also shortened to `Serial number, or describe it…` (225px in a 291px box) because the old one was clipped; **it is duplicated in `guitar-identifier.astro` and `guitar-identifier.ts`, keep them in sync.** |
