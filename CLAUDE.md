...
| 2026-07-27 | Video embeds + VideoObject/Clip schema across the video pages (Fender hub + pot-codes + neck-dates, Gibson & Martin serial guides, free-appraisal, homepage `Guide.astro` shipping video, and the sell-* pages) | **Extended the existing chapter-deep-link pattern site-wide, fixed two wrong timestamps, and added VideoObject + Key Moments schema.** (1) **Fixed timestamps:** pot-codes embed `start=254` → **184** (the "Checking Pots" chapter) and neck-dates `114` → **125** ("Neck Date"), each in all three spots (lite-youtube `params` + VideoObject `embedUrl` + `contentUrl`). (2) **Section deep-link embeds** where a section maps to a chapter: Gibson — Interior Label card → `start=12`, Back-of-Headstock card → `start=28`; Martin — "Where Is The Serial Number Located?" → `start=20`. Each new embed follows the pot-codes pattern (`params="rel=0&modestbranding=1&start=NNN"` + a local poster background via `assetSrc`, reusing that section's own photo). The **Fender hub got NO new section embeds** — its Pot-Codes/Neck-Heel cards link to the spoke pages, which already carry the deep-linked embeds (Joe's call); the hub's 0:00 embed was left alone. (3) **VideoObject** (copied the hub shape: `author` → `/about-me/#person`, real `uploadDate` pulled from each video's YouTube page, `maxresdefault` thumbnail) added to the 11 embeds that had none. (4) **Key Moments (`hasPart` Clip[])** added to the Fender (8), Gibson (4), and Martin (2) VideoObjects, one Clip per chapter, `url` = page `?t=NNN`. | P1 of the schema/share/video roadmap (`docs/schema-and-share-card-roadmap.md`) wanted VideoObject wherever a video embeds; deep links + Key Moments are an *explicit* signal (Google may already derive Key Moments for YouTube-hosted video, so no rich-result promised). **The Gibson 0:12/0:28 mapping was Joe-confirmed by hand** — the auto chapters are only "Serial Number Location 1/2" and the video's own description lists them in the reverse (headstock-first) order, so it was verified, not guessed. **The "Meet Joe" clip (`zMd6IJ-6e-8`, whose real YouTube title is a 1962 ES-335 performance) is named per each page's on-page embed title** ("Meet Vintage Guitar Expert, Joe Dampt"; Martin sell page uses "Introducing Joe Dampt, A Martin Guitar Connoisseur") to match how the site presents it, not the raw title — flagged for Josh. Gates: `astro check` 0 errors; build 116 pages (117 locally only because the untracked `how-to-spot-a-fake-vintage-fender` draft builds too); `verify-image-license.mjs` 209 nodes / 0 mismatches. |
