...
8. **Keep the sitemap current, every change (not launch-only).** Whenever you add, relaunch, or meaningfully edit any page or post, make the sitemap reflect it before calling the work done. This site uses `@astrojs/sitemap` (auto-generated), so a rebuild + redeploy regenerates `sitemap-index.xml` with the new route (the `build` script also regenerates `image-sitemap.xml`). Bump the page's schema `dateModified` (and any visible "Updated" date) to the change date so Google doesn't print a stale date in the SERP. Then re-submit the sitemap in Google Search Console (`mcp__gsc__submit_sitemap`) and confirm it processed. Two gotchas: GSC can hold STALE sitemap records from the old WP/Yoast era, so a re-submit forces re-processing; and a sitemap INDEX reading "0 pages discovered" is NORMAL (the index has no URLs of its own, the children carry them), so check the children rather than redoing the sitemap. For the image sitemap specifically, see [The image sitemap must be DISCOVERABLE](#the-image-sitemap-must-be-discoverable-not-just-generated).
